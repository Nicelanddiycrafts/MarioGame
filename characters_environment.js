...
function start(){
  gameStatus="start";
  document.getElementById('a').innerHTML = "Game is loading";
}
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press play to start and move your nose to control the game", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  ...
  if((gameStatus=="start" && nosex!="" && gameConfig.status==="start")) {
    document.getElementById('a').innerHTML = "Game is loaded";
    world_start.play();
